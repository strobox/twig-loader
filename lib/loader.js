...
function genStyleds(styles) {
    let idx = -1;
    return styles.map( ({tag,css}) => {
        idx++;
        return `
            var StyledCmp_${idx} = styled(${tag})\`
                ${css}
            \``
    }).join(';\n');
}
module.exports = function(source) {
    ...
    var res = `
        ...
        ${ haveStyles ? genStyleds(tpl.styleBlocks) : ''}
        ...
        ${ extendsTpl ? `export default p => parent(Object.assign({twig_blocks:${comp.blocksStr}},p))`
                : `export default p => ${cmpStr||'null'};`
        }
